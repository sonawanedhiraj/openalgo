<!-- migrated from outputs/2026-06-11_upstream_e2e_test_audit.md on 2026-06-13 | summary: Upstream OpenAlgo e2e/test audit â€” 2026-06-11 -->

# Upstream OpenAlgo e2e/test audit â€” 2026-06-11
...
`test/sandbox/` dir, etc.). They are not "adopt" candidates â€” they're our own
inheritance.
...
| --- | --- | --- | --- | --- | --- |
| `test_mode_normalization.py` | `websocket_proxy/mode_utils.py::normalize_mode` | âœ… both fns present | âœ… pure function, zero deps | âœ… **34/34 pass** | **ADOPT** |
| `test_auth_resume.py` | `blueprints/auth.py::_try_resume_broker_session`, `login()` | âœ… `auth.py:137` (used at :272, :370) | âœ… monkeypatch, no broker/net | âŒ 4/6 fail â€” **fork diverged** | **ADAPT** |
| `test_session_expiry.py` | `utils/session.py::revoke_user_tokens` | âœ… `session.py:97` | âœ… monkeypatch, fake socketio | âŒ fail â€” **fork diverged** | **ADAPT** |
| `test_multi_option_greeks_regression.py` | `services/option_greeks_service.py` | âœ… both fns present | âœ… monkeypatch, no net | âŒ 2/2 fail â€” **fork diverged** | **ADAPT** |
| `test_dhan_margin_api.py` | `broker/dhan/api/margin_api.py`, `mapping/margin_data.py` | âœ… files present | âœ… monkeypatch | (not run â€” broker we don't trade) | **SKIP** |
...
`monkeypatch`-ed â€” no live broker, no network, no real DB). None expect a running
...
> *running* them against our code (the right move â€” signatures existing â‰  behavior
...
> and porting the upstream production fix is beyond a test-adoption task â€” so they
> drop to ADAPT, and the divergences are reported below as findings (the real value
> of this audit).

## ADOPT â€” value rationale

1. **`test_mode_normalization.py`** (34 parametrized cases, all green) â€” Pure-function
   ...
   heavily (scanner self-subscribe, pre-subscribe boot race) â€” cheap insurance on a
   primitive our scanner path depends on.

## Divergence findings â€” the 3 ADAPT tests (what running them surfaced)
...
1. **`_try_resume_broker_session` (auth resume) â€” robustness gap, plausible latent bug.**
   ...
   dict like `{"status":"error","message":"token expired"}` is **non-empty â†’
   ...
   **resume the session**. For Zerodha (which returns `{}` on failure) this is
   ...
   the test alongside it. (The 2 `login()` cases also fail with `AttributeError` â€”
...
2. **`revoke_user_tokens` (session expiry) â€” missing upstream feature.**
   ...
   **no SocketIO events**. Not a bug â€” a feature we never pulled. Low urgency (single
   self-hosted operator, rarely multi-device), but cheap to port if multi-device
   force-logout is ever wanted.

3. **`get_multi_option_greeks` / `calculate_time_to_expiry` (option greeks) â€” graceful expired-leg handling we lack.**
   ...
   (`0.000599â€¦` vs `0.0`) for the same input and propagates the per-leg error instead
...
- **`test_dhan_margin_api.py`** â€” Hermetic and would pass (the Dhan margin code
  ...
  change surface â€” only maintenance burden on a broker we don't use. Honest call:
  no value, skip.

- **The ~58 name-matching files** â€” already in our `test/` (we forked from
...
## Integration notes (Phase 2 â€” what actually landed)

- **Adopted: 1 file â†’ `test/upstream/test_mode_normalization.py`** (34 parametrized
...
- Minimal porting: upstream resolves its module path via
  ...
  `test/upstream/` that became `parents[2]` â€” the only functional edit. Added
  `pytestmark = pytest.mark.unit`.
- RELIANCE phantom-row baseline in live `db/openalgo.db` before/after the run:
  **32 â†’ 32** (conftest-guard verification â€” pytest could not touch the live DB).

## Follow-ups (out of scope tonight)

- **Fix the auth-resume robustness gap** (finding #1) â€” make
...
- A broader `git diff upstream/main -- test/` sweep on the ~58 *shared-name* files
  could surface upstream test improvements to our inherited files â€” separate effort.
