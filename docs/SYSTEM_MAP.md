...
### 5b. CasStraddleService (in-process, OpenAlgo eventlet worker, issue #740)
- **Entry:** `services/cas_straddle_service.py` — built + wired at boot by
  `init_cas_straddle_service(app)` (called from `app.py`, **unconditional — no env
  flag**). Expiry-day ATM straddle across the closing auction: NIFTY (NSE_INDEX
  spot, NFO options, Tuesdays) and SENSEX (BSE_INDEX spot, BFO options,
  Thursdays); the expiry day comes from the master contract
  (`expiry_service.get_expiry_dates`), never from `weekday()`.
- **Mode:** exactly like open15 — `sandbox` or `live`, decided per order by
  `resolve_order_mode('cas_320_expiry_straddle')` (the `/strategies` toggle →
  `strategy_mode` row; default-deny sandbox; the navbar Analyze toggle forces
  sandbox). No observe/scaffold state.
- **Controls:** the `/strategies` toggle, the Settings card
  (`cas_straddle_config`: per-underlying trade toggles, lots, premium cap, target
  multiple, hard exit time, poll interval — see `docs/PARAMETER_LOG.md`), and
  `pause`/`resume` (`strategy_runtime_override`).
- **Jobs** (shared scheduler, `mon-fri` `Asia/Kolkata`, `replace_existing`):

  | Job id | Cron (IST) | What it does |
  |---|---|---|
  | `cas_straddle_daily_reset` | 09:00 | Clear the per-day state (legs, ladder, samples) |
  | `cas_straddle_arm` | 15:12 | Trading day + expiry day per underlying, config snapshot, ATM ±2 ladder from the spot, monitor thread start, hard-exit job re-schedule; idle on a non-expiry day |
  | `cas_straddle_entry` | 15:20:00 | BUY ATM CE + PE (`NRML`, `lots × lotsize`; sandbox MARKET, live marketable LIMIT at ask + 5 ticks) per underlying toggled on; premium above `max_premium_inr` is refused, never trimmed; every leg verified through the order book (an ACK is not a fill) — a rejected leg is `fill='paper'` |
  | `cas_straddle_hard_exit` | `hard_exit_time` (default 15:28:00) | SELL any leg with a bid; a leg with no bid is left to cash-settle (`expired_worthless`) |
  | `cas_straddle_fallback_flatten` | 15:38 | PROTECTED: SELL whatever the position book (`mode_key`-resolved, #497) still affirmatively holds; an unreadable book still sends for a confirmed fill, never for an unverified one |
  | `cas_straddle_eod_summary` | 15:45 | Session digest per underlying + settlement counterfactual per leg + Telegram |

- **Thread:** `cas-straddle-monitor` (daemon, 15:14–15:41 IST on an expiry day,
  cadence = `poll_interval_s`) — ONE `get_multiquotes` call per cycle for spot +
  the ATM ±2 ladder of both underlyings (whether or not they trade), journals
  every poll, fixes the ATM from the 15:15:00 spot print, tracks the indicative
  index path, and evaluates the combined-bid target on the same batch. Calls
  `thread_registry.done()` when the window closes.
- **DBs written:** `db/openalgo.db` → `cas_straddle_config`, `cas_straddle_trades`
  (one row per option LEG), `cas_straddle_polls` (~800 polls × 11 symbols per
  underlying-day), `cas_straddle_sessions` (one digest per date+underlying);
  `strategy_runtime_override` (pause); plus order rows in `db/sandbox.db` when
  sandbox.
- **Control API:** `/cas_320_expiry_straddle/api/*` (see "Strategy control
  endpoints" below). **UI:** `/strategies/cas_320_expiry_straddle` Settings /
  Today / Expiry sessions cards.
- **Status:** ACTIVE in sandbox from the first expiry day after merge
  (2026-09-22 NIFTY / 2026-09-24 SENSEX). A measurement deployment — no
  historical weekly index-option data exists; the polls/sessions tables are the
  R69 backtest input. Pre-registered kill: a SEBI/NSE change hiding the
  indicative index or moving expiry settlement to a VWAP. See
  `strategies/cas_320_expiry_straddle/PLAN.md`.
...
### 7. Machine-wide system health (inside `HealthCollector`, issue #744)

Durable fix for the **2026-09-24 09:40:09 IST** libzmq WSAENOBUFS (10055) abort,
which this time took the **whole process** down (Flask, ZMQ :5555 and WS :8765
share one PID), so no in-process watchdog could alert. The existing Health
Monitor had recorded free system RAM falling 533 → 237 MB in the ten minutes
before it; nobody saw it. Socket buffers come from the Windows **non-paged pool**
(machine-wide, must stay in physical RAM — the page file does not help), so the
signals are machine-wide, not OpenAlgo's own process numbers.

- **Where it runs:** no new thread. The existing 10 s `HealthCollector`
  (`utils/health_monitor.py`) calls `utils/system_health.py` each tick:
  `read_system()` (Windows `GetPerformanceInfo`, no admin: free RAM, non-paged
  pool, commit/limit, system handles; plus system-wide and OpenAlgo TCP via
  psutil) and, once a minute, `collect_processes()` + `classify_apps()`.
- **Rules** (`SystemHealthMonitor`): free RAM (<400 WARN / <250 CRIT), non-paged
  pool (>1 GB or >+100 MB/h WARN / >1.5 GB CRIT), system TCP (>2000 WARN). A
  level must hold `SYSTEM_HEALTH_SUSTAIN_SAMPLES` (3 = 30 s). One Telegram per
  (rule, level) per IST day via `notify("system_health")`, WARN→CRIT escalation,
  a RECOVERED message after an alert; `health_alerts` rows are always written.
  **Telegram threshold alerts ship OFF** (`SYSTEM_HEALTH_ALERTS_ENABLED=false`)
  for 3–5 calibration days.
- **Free-up-memory advice:** processes grouped by app, ranked by **working set**
  (the RAM closing frees — committed memory overstates it: the Claude Cowork VM
  `vmmem` holds ~3 GB committed but ~200 MB resident). Never lists OpenAlgo, its
  children or ancestors (the terminal running it), Windows core or Defender.
  **Recommend only — nothing ever terminates a process.**
- **Unclean-exit detection** (`RunStateTracker`): `db/health_run_state.json` is
  marked `running` at boot (BEFORE the collector writes, so the newest samples
  still belong to the previous run) and `clean` by `atexit`. `abort()` skips
  `atexit`, so `running` at the next boot = the previous run died → report in
  `db/health_unclean_exit.json` (last sample, lowest free RAM in its final 5 min,
  top memory users) and, ~90 s after boot on the collector thread, the Windows
  Application-log Event 1000 for the old pid (`wevtutil`, e.g. `0x40000015` =
  abort) + a Telegram (gated only by `NOTIFY_SYSTEM_HEALTH`). A Ctrl+C / SIGINT
  shutdown is clean (atexit runs) and is not reported.
- **UI:** navbar dot (`SystemHealthDot`, every page → `/health`) polling
  `GET /health/api/system` every 30 s with a plain `fetch` (the shared webClient
  redirects on 401). The route is gated `"user" in session` + plain 401 — never
  `check_session_validity`, whose failure path revokes broker tokens. `/health`
  gains the unclean-exit banner (dismiss:
  `POST /health/api/system/unclean_exit/dismiss`), the **System** card with a 6 h
  free-RAM / non-paged chart, and **Free up memory**.
- **Storage:** new `health_metrics` columns (see Databases), added to existing
  installs by `ensure_health_metric_columns()` (create_all never alters a table).
...
| Class | Members | Notes |
|---|---|---|
| **Recurring loops** | `ScannerBackfillPeriodic` (30 min, 15:30-17:00), `ScannerStragglerRecheck` (15 min, 09:20-15:30), `SectorFollowBackfillPeriodic` (30 min, 15:30-17:00), `OptionLiquidityConvergence` (20 min, after 15:55 on trading days — re-runs the option-liquidity sweep if today has no score rows, issue #589), `BrokerAutoLoginWatcher` (5 min, 06:15-23:30 IST — headless re-login on a confirmed dead broker session, primary + children; issue #654, `BROKER_AUTO_LOGIN_ENABLED` default OFF; never initiates a login before `AUTO_LOGIN_EARLIEST_TIME` 07:30 IST because Kite flushes tokens 06:45–07:30, and confirms a pre-flush token's death on the first dead probe — issue #719), `TickLivenessWatchdog` (30 s), `WSProxySupervisor` (30 s), `ScannerWsWatchdog` (60 s), `ThreadWatchdog` (30 s), `HealthCollector` (10 s), `MarketDataHealthCheck` (5 s), `MarketDataCleanup` (300 s), `FlowPriceMonitor` (5 s), `cas-straddle-monitor` (2–60 s UI-set, 15:12–15:41 IST on expiry days only — issue #740) | Cron jobs in all but name. Each stamps `thread_registry.beat()` at the top of its tick — that heartbeat, not `is_alive()`, is what proves the loop is working. |
...
| **Boot one-shots** | `ScannerBackfillBoot`, `SectorFollowBackfillBoot`, `ScannerAggregatorSeed`, `ScannerPreSubscribe`, `RegimeSectorPreSubscribe`, `ScannerHistoryWarmup`, `AbandonedExitRecovery`, `futures_follow_rehydrate`, `IntradayPullbackBootResume`, `WhatsAppAutoStart`, `BrokerAutoLoginBoot` (issue #654 — auto-login once if the broker session is dead at boot, then start `BrokerAutoLoginWatcher`; `BROKER_AUTO_LOGIN_ENABLED`; a boot before `AUTO_LOGIN_EARLIEST_TIME` only logs a deferral and leaves the login to the watcher's first tick past 07:30, issue #719) | Run once and exit; `completed` is the healthy state, not `dead`. |
...
|---|---|---|---|
| `cas_straddle_daily_reset` / `cas_straddle_arm` / `cas_straddle_entry` / `cas_straddle_hard_exit` / `cas_straddle_fallback_flatten` / `cas_straddle_eod_summary` | `0 9`, `12 15`, `0 20 15`, `hard_exit_time` (UI), `38 15`, `45 15` `* * 1-5` | **cas_320_expiry_straddle** (issue #740) — expiry-day ATM straddle on NIFTY/SENSEX; see Processes §5b for the per-job detail | **No env gating** (unconditional at boot). Live vs sandbox = the `/strategies` toggle. Writes `cas_straddle_*` tables + `strategy_runtime_override` |
...
|---|---|---|
| `db/health.db` → `health_metrics` | 10 s samples incl. machine-wide free RAM / non-paged pool (issue #744) | **First stop after an unexplained process death** — the newest rows are the seconds before it; `db/health_unclean_exit.json` has the summary |
...
Blueprint `blueprints/cas_straddle.py`, URL prefix `/cas_320_expiry_straddle` (issue #740).
Mutating endpoints are API-key authenticated; the read-only endpoints AND the
config endpoint also accept a valid logged-in browser session (the React Settings
card). `@check_session_validity` is deliberately not used (its failure path
revokes broker tokens).

| Endpoint | Method | Side effect |
|---|---|---|
| `/cas_320_expiry_straddle/api/status` | GET | Read-only: mode, effective config, today's per-underlying state (expiry day / nearest expiry, ATM, contracts, legs, last target evaluation, poll count), monitor liveness |
| `/cas_320_expiry_straddle/api/config` | GET / POST | GET `{defaults, override, effective, lotsizes}`; POST validates (refuses out-of-bounds, never clamps) + upserts the single `cas_straddle_config` row; applies at the next 15:12 arm |
| `/cas_320_expiry_straddle/api/positions` | GET | Read-only: today's (or `?date=`) journal legs, all fill classes |
| `/cas_320_expiry_straddle/api/sessions` | GET | Read-only: per-(date, underlying) session digests, newest first |
| `/cas_320_expiry_straddle/api/polls` | GET | Read-only: raw poll rows for `?date=` (the research dataset) |
| `/cas_320_expiry_straddle/api/pause` / `resume` | POST | Durable `strategy_runtime_override` pause (same-day expiry) holding the 15:20 entry; exits and the fallback flatten never gated |
| `/cas_320_expiry_straddle/api/close_all` | POST | Emergency SELL of every open leg (`{"confirm":"yes"}`) |
...
2. **Token persist** — `handle_auth_success` calls
   ...
   `auth` table (`db/openalgo.db`) with a `token_updated_at` stamp (#719), clears local auth caches, and **publishes a ZMQ
