...
is now **FIXED** too (Phase B, 2026-06-11) — it sources the effective mode from
...
| Open `trending_equity_intraday` rows | **1 at audit time → 0 after the 06-09 ONGC reconciliation** |
| Total open (`exited_at IS NULL`) rows | **1 at audit time → 0 after reconciliation** |
...
- **The one open row is NOT a phantom — it is a real position.**
  ...
  > ✅ **Reconciled (2026-06-11):** the open ONGC `id=40` row was closed via
  > `uv run python -m services.engine_eod_reconciliation_backfill --from 2026-06-09 --to 2026-06-09 --apply`
  > — sandbox covering BUY fill priced the exit at `259.0`
  > (`exit_reason='sandbox_eod_squareoff'`, gross P&L −₹115.80, order
...
- **Phase B — done (2026-06-11):** migrated `blueprints/mode_status.py` to source
