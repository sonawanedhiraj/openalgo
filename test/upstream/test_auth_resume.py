...
Adopted verbatim from upstream marketcalls/openalgo (test/test_auth_resume.py).
Only the module-path depth was adjusted: this file lives one directory deeper
(``test/upstream/``), so the repo-root resolution uses ``parents[2]`` instead of
``parents[1]``. DB isolation is provided by the global ``test/conftest.py`` guard
(commit fcf96b91) — the ``DATABASE_URL`` setdefault below is a no-op under it.

These tests pin the fork-divergence fix in ``fix(auth): reject structured-error
funds payloads in broker session resume`` — see
``outputs/2026-06-11_upstream_e2e_test_audit.md`` (finding #1).
"""
...
TEST_DB = Path(__file__).resolve().parents[2] / "tmp" / "test_auth_resume.db"
...
def test_login_clears_expired_existing_session_before_password_flow(monkeypatch):
    ...
    with app.test_request_context(
        "/auth/login",
        method="POST",
        data={"username": "rajandran", "password": "password"},  # pragma: allowlist secret
    ):
        session["user"] = "rajandran"
        session["logged_in"] = True
        session["broker"] = "dhan"
