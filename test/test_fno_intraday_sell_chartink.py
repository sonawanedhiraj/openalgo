...
def test_fail_snapshot_carries_failed_gate():
    """A known-gate failure (gap-down gate, g1) stashes the exact gate name."""
    assert rule(None, _fail_g1()) is False
    snapshot = rulemod.get_last_fail_snapshot()
    assert snapshot is not None
    assert snapshot["failed_gate"] == "gap_down"
...
def test_fail_snapshot_reflects_latest_gate():
    ...
    rule(None, _fail_g1())
    assert rulemod.get_last_fail_snapshot()["failed_gate"] == "gap_down"
    rule(None, _fail_g5())
