...
**Broker re-login → WS reinit (event-driven, no restart, no flag).** Indian broker tokens expire daily ~3 AM IST. The WS proxy runs as a **separate subprocess** from the Flask app, so a Flask SocketIO event cannot reach it — the cross-process signal is the ZMQ `CACHE_INVALIDATE` event that `database.auth_db.upsert_auth()` publishes on every re-login. `WebSocketProxy._handle_cache_invalidation` consumes it and **unconditionally** calls `_reconnect_broker_adapter(user_id)`: it snapshots the held symbol subscriptions, disconnects, re-reads the fresh token via `adapter.initialize()`, reconnects, and re-subscribes — so the feed resumes **without an OpenAlgo restart**. It is failure-graceful (a rejected token logs `logger.exception`, retains the subscription snapshot in `_last_known_subscriptions`, and drops the dead adapter for the next client auth to rebuild) and idempotent (repeated events reuse the one adapter; disconnect always precedes reconnect). **The snapshot must read BOTH adapter shapes** (issue #673): raw adapters track `subscribed_symbols` while the pooled wrappers (`ENABLE_CONNECTION_POOLING`, the production default) expose `subscriptions` — reading only the former silently preserved 0 subscriptions on every pooled reconnect and killed the tick feed on 2026-08-25. The reconnect then verifies the restore against the proxy's own client-driven `subscription_index` (which survives an adapter wipe) and re-subscribes any shortfall from it — additive-only, no flag (a correctness guard is not a preference). On **login-driven** reconnects the index restore is the PRIMARY mechanism, not a fallback: the login flow disconnects the pool (clearing `subscription_map`) BEFORE the CACHE_INVALIDATE is consumed (live finding, 2026-08-25 10:12:33), so no adapter-side snapshot can survive a re-login — the expected full restore logs WARNING, and only an incomplete restore (some re-subscribes failed → those symbols have no feed) escalates to ERROR. There is **no feature flag** — the E2E suites (`test/test_broker_session_auto_reconnect.py`, `test/test_ws_proxy_pooled_reconnect.py` — the latter uses the REAL `_PooledAdapterWrapper`, because a test fake that drifts from the production adapter shape is exactly how #673 stayed green) carry the safety guarantee. The login completion (`utils/auth_utils.handle_auth_success` → `notify_broker_session_refreshed`) also emits a `broker_session_refreshed` SocketIO event as a **UI/observability notification** (not the reconnect trigger — the subprocess is reached only via ZMQ).
