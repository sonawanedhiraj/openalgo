...
def check_and_refresh_if_stale(
    ...
    *,
    ...
    terminal state, MAX(timestamp) per index is re-read and an index only counts
    as refreshed if its coverage actually advanced. Indices that remain behind
    ...
    try:
        ...
        if is_transient_lock_error(e):
            ...
            logger.info("sector_follow index stale-check skipped — historify briefly locked: %s", e)
    ...
    if bf.get("status") == "success":
        ...
        # only count an index as refreshed if its coverage actually advanced.
        ...
        # reported `errors=0` with every stale index marked refreshed.
        _verify_and_report_refresh(result, bf.get("job_id"), path, stale, ref)
    else:
        logger.warning(
            "sector_follow index catch-up FAILED for %d index(es) — %s — symbols=%s",
            len(stale),
...
def _verify_and_report_refresh(
    ...
    """Wait for the submitted job, then verify each stale index actually advanced.

    Mutates ``result`` in place: ``refreshed`` only contains indices whose
    ...
    if job_id:
        try:
            ...
            logger.exception(
                "sector_follow index catch-up: wait_for_jobs raised for job_id=%s — "
    ...
    try:
        ...
        logger.exception(
            "sector_follow index catch-up: post-job freshness verification failed (%s) — "
            "falling back to submission-based reporting for %d index(es)",
            e,
            len(stale),
        )
    ...
    logger.info(
        "sector_follow index catch-up verified: verified_fresh=%d still_stale=%d",
        len(verified_fresh),
        len(still_stale),
    )
    ...
    if still_stale_pct > 0.20:
        msg = (
            f"sector_follow index catch-up: {len(still_stale)}/{len(stale)} "
            f"({still_stale_pct:.0%}) indices still stale after a completed job — "
