...
def cancel_order_with_auth(
    ...
    # Order-management routing (issue #440): Analyze ON → sandbox only (the
    # broker is never touched). With Analyze OFF the books can be mixed, so
    # route by where the order actually lives — a sandbox orderid goes to the
    # sandbox book, anything else falls through to the broker.
    ...
    route_sandbox = mode is EffectiveMode.SANDBOX or (
        original_data.get("apikey") and sandbox_order_exists(orderid, original_data["apikey"])
    )
    ...
    # Not a sandbox-book order and Analyze is off — proceed with broker cancel.
...
def cancel_order(
    ...
    if api_key and not (auth_token and broker):
        ...
        # BUT allow execution while Analyze mode is on (sandbox always works).
