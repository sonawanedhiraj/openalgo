...
def cancel_order_with_auth(
    ...
    if mode is EffectiveMode.SKIP:
        logger.info("Cancel order rejected: daily_intent is 'skip' for today.")
        rejection = {
            "status": "rejected",
    ...
    if mode is EffectiveMode.DISABLED:
        logger.warning("Cancel order rejected: no daily_intent declared for today.")
        rejection = {
            "status": "rejected",
    ...
    # mode is EffectiveMode.LIVE — proceed with actual broker cancel.
